...
EOF
    ...
    fr=$(jq '."insecure-registries" as $r | "'"$openshift_local_url"'" | IN($r[])' < "$docker_daemon_file")
    if [ "$fr" = "false" ]; then
        ...
        jq '."insecure-registry += ["'"$openshift_local_url"'"]' < "$docker_daemon_file.bkp" | sudo tee "$docker_daemon_file"
...
prepare_cluster() {
    ...
    local openshift_local_url
    openshift_local_url=$(oc get route default-route -n openshift-image-registry --template='{{ .spec.host }}')
...
main() {
    ...
    ## wait for rollout
    loop_for 20 30 check_deployment_rollout "${SERVICES[*]}" -n "$NAMESPACE" || return 1
